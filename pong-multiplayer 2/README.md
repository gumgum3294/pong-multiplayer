...
- Deploy on [Railway](https://railway.app) or [Render](https://render.com) (both have free tiers)
- Set `PORT` environment variable if needed (the server reads `process.env.PORT`)
...
├── public/
│   └── index.html     # Client game (HTML + CSS + JS)
├── package.json
└── README.md
```
...
- **Game Logic**: Server-authoritative at 60 ticks/second
